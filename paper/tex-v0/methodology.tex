...
A good model should rank the correct receiver among the top of the ranked list. \textbf{LambdaRank}~\cite{burges2010ranknet} is a general learning to rank framework which is widely used for learning to rank tasks. It can work with lots of underlying regression models. Among those, gradient boosted decision trees (GBDT) is a common choice to achieve high performance. The combination of LambdaRank and GBDT is usually termed \textbf{LambdaMART}~\cite{burges2010ranknet}.
...
In this work, we use an efficient implementation of LambdaMART, \textbf{LightGBM}~\cite{NIPS2017_6907}, which speeds up the training time of conventional LambdaMART (e.g., XGBoost and pGBRT) by up to 20 times while achieving almost the same accuracy. 
...
Gradient boosted trees (e.g., LightGBM) provide a straightforward way to retrieve the importance scores of each feature~\cite{friedman2001elements}.

After the boosted decision trees are constructed, the importance of a feature is calculated by accumulating all the split gains (i.e., Gini index) that the feature has contributed to the model.
